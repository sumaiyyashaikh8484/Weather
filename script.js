// API Key
const API_KEY = "a6ebfa5e199ef08d4124da6ac6e80f2b";
...
function renderWeatherInfo(weatherInfo) {
    ...
    clouds.innerText = `${weatherInfo?.clouds?.all.toFixed(2)} %`;
     ...
     var temp_alert=`${weatherInfo?.main?.temp.toFixed(2)}`;
    ...
    var wind_alert=`${weatherInfo?.wind?.speed.toFixed(2)}`;
    
    if(wind_alert>=15){
        alert("High windspeed Detected");
        var x=document.querySelector(".wind_alert");

        x.play();
    }
     

    var rain_alert=`${weatherInfo?.clouds?.all.toFixed(2)}`;
    if(rain_alert>=80){
        alert("Rain detected");
        var x=document.querySelector(".wind_alert");
        x.play();
